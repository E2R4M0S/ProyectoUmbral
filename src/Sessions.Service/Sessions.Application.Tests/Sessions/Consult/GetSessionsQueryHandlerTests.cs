...
public class GetSessionsQueryHandlerTests
{
    ...
    public async Task Handle_ReturnsItemsAndTotalCount()
    {
        ...
        {
            Session.Create("Session A", "111111", new List<SessionStage> { SessionStage.Create(Guid.NewGuid(), Guid.NewGuid(), "Test Mission", "Stage", "Trivia", 1, "test-token") }),
            Session.Create("Session B", "222222", new List<SessionStage> { SessionStage.Create(Guid.NewGuid(), Guid.NewGuid(), "Test Mission", "Stage", "Trivia", 1, "test-token") }),
        };
    ...
    public async Task Handle_AppliesSearchFilter_CaseInsensitive()
    {
        ...
        var sessions = new List<Session>
        {
            Session.Create("Team Alpha Session", "333333", new List<SessionStage> { SessionStage.Create(Guid.NewGuid(), Guid.NewGuid(), "Test Mission", "Stage", "Trivia", 1, "test-token") }),
        };
    ...
    {
        // Arrange
        var session = Session.Create("Active Session", "444444", new List<SessionStage> { SessionStage.Create(Guid.NewGuid(), Guid.NewGuid(), "Test Mission", "Stage", "Trivia", 1, "test-token") });
    ...
    public async Task Handle_AppliesMissionIdFilter()
    {
        ...
        var sessions = new List<Session>
        {
            Session.Create("Mission Session", "555555", new List<SessionStage> { SessionStage.Create(missionId, Guid.NewGuid(), "Test Mission", "Stage", "Trivia", 1, "test-token") }),
        };
    ...
    public async Task Handle_WithPagination_ReturnsCorrectPage()
    {
        ...
        for (int i = 0; i < 5; i++)
        {
            sessions.Add(Session.Create($"Session {i}", $"{100000 + i}", new List<SessionStage> { SessionStage.Create(Guid.NewGuid(), Guid.NewGuid(), "Test Mission", "Stage", "Trivia", 1, "test-token") }));
        }
    ...
    public async Task Handle_WithInvalidPage_DefaultsToPage1()
    {
        ...
        var sessions = new List<Session>
        {
            Session.Create("Session A", "666666", new List<SessionStage> { SessionStage.Create(Guid.NewGuid(), Guid.NewGuid(), "Test Mission", "Stage", "Trivia", 1, "test-token") }),
        };
    ...
    public async Task Handle_WithInvalidPageSize_DefaultsTo10()
    {
        ...
        var sessions = new List<Session>
        {
            Session.Create("Session A", "777777", new List<SessionStage> { SessionStage.Create(Guid.NewGuid(), Guid.NewGuid(), "Test Mission", "Stage", "Trivia", 1, "test-token") }),
        };
