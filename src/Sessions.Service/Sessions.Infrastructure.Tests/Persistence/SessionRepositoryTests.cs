...
public class SessionRepositoryTests
{
    ...
    public async Task IsPinUniqueAsync_WhenPinExists_ReturnsFalse()
    {
        ...
        var existingSession = Session.Create("Existing", "111111", new List<SessionStage> { SessionStage.Create(Guid.NewGuid(), Guid.NewGuid(), "Test Mission", "Trivia", 1, Guid.NewGuid().ToString("N")) });
    ...
    public async Task AddAsync_ShouldSetCreatedAt()
    {
        ...
        var session = Session.Create("Timed Session", "222222", new List<SessionStage> { SessionStage.Create(Guid.NewGuid(), Guid.NewGuid(), "Test Mission", "Trivia", 1, Guid.NewGuid().ToString("N")) });
    ...
    public async Task IsPinUniqueAsync_WithMultipleSessions_ChecksCorrectly()
    {
        ...
        var session1 = Session.Create("Session 1", "333333", new List<SessionStage> { SessionStage.Create(Guid.NewGuid(), Guid.NewGuid(), "Test Mission", "Trivia", 1, Guid.NewGuid().ToString("N")) });
        var session2 = Session.Create("Session 2", "444444", new List<SessionStage> { SessionStage.Create(Guid.NewGuid(), Guid.NewGuid(), "Test Mission", "Trivia", 1, Guid.NewGuid().ToString("N")) });
    ...
    public async Task GetByIdAsync_WhenSessionExists_ReturnsSessionWithParticipants()
    {
        ...
        var session = Session.Create("Existing Session", "666666", new List<SessionStage> { SessionStage.Create(Guid.NewGuid(), Guid.NewGuid(), "Test Mission", "Trivia", 1, Guid.NewGuid().ToString("N")) });
    ...
    public async Task GetByPinAsync_WhenPinExists_ReturnsSession()
    {
        ...
        var session = Session.Create("Pin Session", "777777", new List<SessionStage> { SessionStage.Create(Guid.NewGuid(), Guid.NewGuid(), "Test Mission", "Trivia", 1, Guid.NewGuid().ToString("N")) });
        dbContext.Sessions.Add(session);
        await dbContext.SaveChangesAsync();
    ...
    public async Task GetByNameAsync_WhenNameExists_ReturnsSession()
    {
        ...
        var session = Session.Create("Named Session", "888888", new List<SessionStage> { SessionStage.Create(Guid.NewGuid(), Guid.NewGuid(), "Test Mission", "Trivia", 1, Guid.NewGuid().ToString("N")) });
        dbContext.Sessions.Add(session);
        await dbContext.SaveChangesAsync();
    ...
    public async Task GetSessionsAsync_WithNoFilters_ReturnsAllSessionsPaginated()
    {
        ...
        var sessions = new[]
        {
            Session.Create("Session A", "111111", new List<SessionStage> { SessionStage.Create(missionId, Guid.NewGuid(), "Mission", "Trivia", 1, Guid.NewGuid().ToString("N")) }),
            Session.Create("Session B", "222222", new List<SessionStage> { SessionStage.Create(missionId, Guid.NewGuid(), "Mission", "Trivia", 1, Guid.NewGuid().ToString("N")) }),
            Session.Create("Session C", "333333", new List<SessionStage> { SessionStage.Create(missionId, Guid.NewGuid(), "Mission", "Trivia", 1, Guid.NewGuid().ToString("N")) }),
        };
    ...
    public async Task GetSessionsAsync_WithSearchFilter_ReturnsMatchingSessions()
    {
        ...
        var missionId = Guid.NewGuid();
        dbContext.Sessions.Add(Session.Create("Alpha Session", "444444", new List<SessionStage> { SessionStage.Create(missionId, Guid.NewGuid(), "Mission", "Trivia", 1, Guid.NewGuid().ToString("N")) }));
        dbContext.Sessions.Add(Session.Create("Beta Session", "555555", new List<SessionStage> { SessionStage.Create(missionId, Guid.NewGuid(), "Mission", "Trivia", 1, Guid.NewGuid().ToString("N")) }));
        dbContext.Sessions.Add(Session.Create("Alpha Beta Session", "666666", new List<SessionStage> { SessionStage.Create(missionId, Guid.NewGuid(), "Mission", "Trivia", 1, Guid.NewGuid().ToString("N")) }));
        await dbContext.SaveChangesAsync();
    ...
    public async Task GetSessionsAsync_WithMissionIdFilter_ReturnsMatchingSessions()
    {
        ...
        dbContext.Sessions.Add(Session.Create("Session 1", "111111", new List<SessionStage> { SessionStage.Create(missionId1, Guid.NewGuid(), "Mission 1", "Trivia", 1, Guid.NewGuid().ToString("N")) }));
        dbContext.Sessions.Add(Session.Create("Session 2", "222222", new List<SessionStage> { SessionStage.Create(missionId2, Guid.NewGuid(), "Mission 2", "Trivia", 1, Guid.NewGuid().ToString("N")) }));
        dbContext.Sessions.Add(Session.Create("Session 3", "333333", new List<SessionStage> { SessionStage.Create(missionId1, Guid.NewGuid(), "Mission 1", "Trivia", 1, Guid.NewGuid().ToString("N")) }));
        await dbContext.SaveChangesAsync();
    ...
    public async Task UpdateAsync_ShouldPersistSessionChanges()
    {
        ...
        var session = Session.Create("Original Name", "999999", new List<SessionStage> { SessionStage.Create(Guid.NewGuid(), Guid.NewGuid(), "Original Mission", "Trivia", 1, Guid.NewGuid().ToString("N")) });
        dbContext.Sessions.Add(session);
        await dbContext.SaveChangesAsync();
    ...
    public async Task AddParticipantAsync_ShouldPersistParticipant()
    {
        ...
        var session = Session.Create("Participant Session", "000001", new List<SessionStage> { SessionStage.Create(Guid.NewGuid(), Guid.NewGuid(), "Test Mission", "Trivia", 1, Guid.NewGuid().ToString("N")) });
        session.TransitionTo(SessionStatus.Preparing);
        dbContext.Sessions.Add(session);
        await dbContext.SaveChangesAsync();
